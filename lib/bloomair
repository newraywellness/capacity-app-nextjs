...
export function BloomScene({ mode, subtle }) {
  ...
  const o = (night ? 0.34 : dark ? 0.42 : 0.5) * (subtle ? 0.62 : 1)
  ...
  const PLOT = [
    { x:18,  h:150, k:"rose",  s:24, d:-2,  r:13 },
    { x:52,  h:104, k:"jas",   s:29, d:-13, r:8 },
    { x:88,  h:186, k:"hyd",   s:22, d:-6,  r:16 },
    { x:132, h:118, k:"rose",  s:27, d:-18, r:10 },
    { x:186, h:88,  k:"jas",   s:31, d:-24, r:7 },
    { x:246, h:164, k:"rose",  s:25, d:-9,  r:14 },
    { x:288, h:110, k:"hyd",   s:23, d:-16, r:12 },
    { x:334, h:196, k:"rose",  s:26, d:-4,  r:15 },
    { x:376, h:96,  k:"jas",   s:30, d:-21, r:8 },
    { x:410, h:142, k:"hyd",   s:28, d:-11, r:13 },
    { x:436, h:172, k:"rose",  s:24, d:-7,  r:11 },
  ]
  ...
  return (
    <div style={{ position:"absolute", left:0, right:0, bottom:0, height:GH, pointerEvents:"none", overflow:"hidden",
      ...
      {/* ── the waterfall — distant, partially hidden, felt more than seen ── */}
      <div style={{ position:"absolute", right:14, top:6, width:104, height:GH-150, opacity:(dark?0.5:0.72)*(subtle?0.7:1), overflow:"hidden", borderRadius:"40% 40% 18% 18%" }}>
        <div style={{ position:"absolute", inset:0, background:`linear-gradient(180deg,${c.foam} 0%,${c.water} 22%,${c.waterDeep} 78%,${c.water} 100%)`, opacity:0.5 }} />
        {[10,30,50,68,86].map((x,i)=>(
          <div key={i} style={{ position:"absolute", left:`${x}%`, top:"-16%", width:i%2?3:2, height:"132%",
            background:`linear-gradient(180deg,rgba(255,255,255,0) 0%,${c.foam} 20%,${c.foam} 80%,rgba(255,255,255,0) 100%)`,
            opacity:0.55, animation:`bwFall ${5+i*1.4}s linear infinite`, animationDelay:`${-i*1.1}s` }} />
        ))}
        <div style={{ position:"absolute", inset:0, background:`linear-gradient(180deg,rgba(255,255,255,0.4),rgba(255,255,255,0))`, animation:"bwShim 7s ease-in-out infinite" }} />
      </div>
      {/* ripples where it lands */}
      {[0,1,2].map((i)=>(
        <div key={i} style={{ position:"absolute", right:44, bottom:GH-BY+34+i*7, width:56, height:7, borderRadius:"50%",
          border:`1px solid ${c.foam}`, opacity:0.4, animation:`bwRipple ${4.5+i*1.6}s ease-out infinite`, animationDelay:`${-i*1.5}s` }} />
      ))}
        ...
        <svg width="100%" height={GH} viewBox={`0 0 440 ${GH}`} preserveAspectRatio="xMidYMax slice" style={{ display:"block" }}>
          <defs>
            <linearGradient id="tr-bloomground" x1="0" y1="0" x2="0" y2="1">
              ...
              <stop offset="100%" stopColor={c.leaf} stopOpacity={(0.24*S.lush).toFixed(3)} />
            </linearGradient>
          </defs>
          ...
          <g opacity={0.72*S.lush}>
            {[[14,26],[74,20],[140,30],[204,22],[268,28],[330,20],[396,26],[434,22]].map(([mx,mh],i)=>(
