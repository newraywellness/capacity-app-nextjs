...
const H0 = {
  ...
  lavender: "#A98BC4", lavStem: "#8E9E7E",
  stem: "#7B9A6A",
...
// A few isolated sprigs beside the hero. Margins only, very faint — these frame
// the top of the page without ever becoming a border.
const HERO_SPRIGS = [
  { x: 12,  y: 304, k: "lavender", h: 152, s: 30, d: -3,  o: 0.55 },
  { x: 40,  y: 340, k: "lavender", h: 104, s: 27, d: -18, o: 0.42 },
  { x: 428, y: 330, k: "mint",     h: 138, s: 26, d: -15, o: 0.52 },
  { x: 404, y: 356, k: "basil",    h: 86,  s: 24, d: -21, o: 0.44 },
  { x: 30,  y: 372, k: "rosemary", h: 74,  s: 28, d: -9,  o: 0.40 },
]
const HERO_BLOSSOM = [
  { x: 400, y: 206, r: 5.5, o: 0.40 },
  { x: 24,  y: 168, r: 4.5, o: 0.34 },
  { x: 420, y: 262, r: 4,   o: 0.30 },
  { x: 44,  y: 226, r: 3.6, o: 0.26 },
  { x: 392, y: 156, r: 4,   o: 0.24 },
]
// Two slices barely on-canvas at the upper corners. Whispers, not decoration.
const HERO_SLICES = [
  { x: -26, y: 104, kind: "orange", size: 40, rot: -18, o: 0.20 },
  { x: 464, y: 152, kind: "lemon",  size: 36, rot: 14,  o: 0.17 },
]
...
export function NourishAir({ mode, tint }) {
  ...
  const hs = herbSeason()
  const _t = (hex) => {
    const seasoned = mixc(hex, hs.tone, 0.26)
    return dark ? mixc(seasoned, night ? "#39324A" : "#6E5D62", night ? 0.48 : 0.26) : seasoned
  }
  const _d = (hex) => (dark ? mixc(hex, night ? "#39324A" : "#6E5D62", night ? 0.42 : 0.22) : hex)
  const hc = {
    rosemary: _t(H0.rosemary), basil: _t(H0.basil), mint: _t(H0.mint), thyme: _t(H0.thyme),
    sage: _t(H0.sage), stem: _t(H0.stem), lavStem: _t(H0.lavStem), lavender: _d(H0.lavender),
    blossom: _d(H0.blossom), blossomCore: _d(H0.blossomCore), seed: _d(H0.seed),
    orange: _d(H0.orange), orangeRind: _d(H0.orangeRind), orangeCore: _d(H0.orangeCore),
    lemon: _d(H0.lemon), lemonRind: _d(H0.lemonRind), lemonCore: _d(H0.lemonCore),
  }
  ...
  return (
    <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 400, pointerEvents: "none", overflow: "hidden" }}>
      ...
      {/* sparse growth at the margins, framing the hero */}
      <svg width="100%" height="400" viewBox="0 0 440 400" preserveAspectRatio="xMidYMin slice" style={{ position: "absolute", inset: 0, display: "block" }}>
        {HERO_SLICES.map((sl, i) => (
          <g key={"hsl" + i} transform={`translate(${sl.x},${sl.y}) rotate(${sl.rot})`} opacity={sl.o * (dark ? 0.55 : 1)}>
            <Slice kind={sl.kind} c={hc} size={sl.size} />
          </g>
        ))}
        {HERO_BLOSSOM.map((b, i) => (
          <g key={"hb" + i} transform={`translate(${b.x},${b.y})`} opacity={b.o * (dark ? 0.5 : 0.75)}>
            {[0, 72, 144, 216, 288].map((r) => <ellipse key={r} cx="0" cy={-b.r} rx={b.r * 0.62} ry={b.r} fill={hc.blossom} transform={`rotate(${r})`} />)}
            <circle cx="0" cy="0" r={b.r * 0.36} fill={hc.blossomCore} />
          </g>
        ))}
        {HERO_SPRIGS.map((p, i) => (
          <g key={"hs" + i} transform={`translate(${p.x},${p.y})`} opacity={p.o * (dark ? 0.45 : 0.7)}>
            <g style={{ transformOrigin: "0px 0px", animation: `${i % 2 ? "hbSway2" : "hbSway"} ${p.s}s ease-in-out infinite`, animationDelay: `${p.d}s` }}>
              <Sprig k={p.k} h={p.h} c={hc} S={hs} />
            </g>
          </g>
        ))}
      </svg>
...
// ── KITCHEN GARDEN ──────────────────────────────────────────────────────────
const GH = 504
const BY = GH - 104   // soil line: stems root here; planting continues below it
...
function Sprig({ k, h, c, S }) {
  ...
  if (k === "lavender") {
    const buds = []
    for (let i = 0; i < 7; i++) {
      const y = -h + i * 5.5, sc = 0.62 + i * 0.06
      buds.push(<ellipse key={i} cx={i % 2 ? 1.8 : -1.8} cy={y} rx={2.6 * sc} ry={3.4 * sc} fill={c.lavender} opacity={(0.55 + i * 0.06) * (0.5 + S.blossom * 0.5)} />)
    }
    return (
      <g>
        <path d={`M0 0 C ${-2} ${-h * 0.44}, ${2} ${-h * 0.72}, 0 ${-h + 34}`} stroke={c.lavStem} strokeWidth="1.4" fill="none" strokeLinecap="round" />
        <ellipse cx={-6} cy={-h * 0.4} rx="7" ry="2.6" fill={c.lavStem} opacity={0.7 * S.foliage} transform={`rotate(-28 -6 ${-h * 0.4})`} />
        {buds}
      </g>
    )
  }
...
const PLOT = [
  { x: 16,  h: 134, k: "rosemary", s: 25, d: -2 },
  { x: 44,  h: 196, k: "lavender", s: 28, d: -13 },
  { x: 68,  h: 78,  k: "thyme",    s: 30, d: -21 },
  { x: 96,  h: 116, k: "sage",     s: 22, d: -6 },
  { x: 124, h: 170, k: "mint",     s: 24, d: -9 },
  { x: 158, h: 92,  k: "basil",    s: 26, d: -17 },
  { x: 192, h: 210, k: "blossom",  s: 27, d: -19 },
  { x: 224, h: 70,  k: "thyme",    s: 31, d: -25 },
  { x: 256, h: 148, k: "lavender", s: 23, d: -4 },
  { x: 286, h: 104, k: "basil",    s: 26, d: -11 },
  { x: 316, h: 182, k: "mint",     s: 21, d: -16 },
  { x: 348, h: 88,  k: "sage",     s: 29, d: -23 },
  { x: 378, h: 158, k: "rosemary", s: 28, d: -7 },
  { x: 406, h: 118, k: "lavender", s: 24, d: -14 },
  { x: 430, h: 190, k: "blossom",  s: 25, d: -10 },
]

export function HerbGarden({ mode, subtle }) {
  const night = mode === "night"
  const dark = mode === "evening" || night
  const S = herbSeason()
  ...
  const c = {
    ...
    lavender: d2(H0.lavender), lavStem: t(H0.lavStem),
  ...
  const o = (night ? 0.34 : dark ? 0.42 : 0.50) * (subtle ? 0.62 : 1)

  return (
    <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, height: GH, pointerEvents: "none", overflow: "hidden",
      maskImage: "linear-gradient(180deg,rgba(0,0,0,0) 0%,rgba(0,0,0,0.6) 14%,rgba(0,0,0,1) 34%)",
      WebkitMaskImage: "linear-gradient(180deg,rgba(0,0,0,0) 0%,rgba(0,0,0,0.6) 14%,rgba(0,0,0,1) 34%)" }}>
      ...
      <div style={{ position: "absolute", inset: 0, opacity: o }}>
        <svg width="100%" height={GH} viewBox={`0 0 440 ${GH}`} preserveAspectRatio="xMidYMax slice" style={{ display: "block" }}>
          ...
          <rect x="0" y={BY - 210} width="440" height={GH - BY + 210} fill="url(#tr-herbground)" />

          {/* low foliage carrying the planting down behind the tab bar */}
          <g opacity={0.7 * S.foliage}>
            {[[10, 30], [64, 22], [118, 34], [172, 24], [226, 32], [282, 20], [336, 30], [392, 26], [432, 22]].map(([mx, mh], i) => (
              <path key={i} d={`M${mx - 30} ${GH} C ${mx - 20} ${BY + 40 - mh}, ${mx} ${BY + 28 - mh}, ${mx + 16} ${BY + 34 - mh} C ${mx + 30} ${BY + 52 - mh}, ${mx + 34} ${GH - 10}, ${mx + 38} ${GH} Z`}
                fill={i % 3 === 0 ? c.sage : i % 3 === 1 ? c.basil : c.thyme} opacity={0.55 + (i % 3) * 0.12} />
            ))}
          </g>

          {/* citrus, mostly off-screen — little discoveries */}
          <g transform={`translate(-14,${BY - 18}) rotate(-13)`} opacity="0.95"><Slice kind="orange" c={c} size={48} /></g>
          <g transform={`translate(452,${BY - 26}) rotate(11)`} opacity="0.92"><Slice kind="lemon" c={c} size={44} /></g>
          <ellipse cx="46" cy={BY - 58} rx="13" ry="6" fill={c.basil} opacity={0.62 * S.citrusLeaf} transform={`rotate(-32 46 ${GH - 66})`} />
          <ellipse cx="416" cy={BY - 68} rx="11.5" ry="5.5" fill={c.basil} opacity={0.55 * S.citrusLeaf} transform={`rotate(28 416 ${GH - 76})`} />
