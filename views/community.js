...
function CommunityApp({ctx}) {
  ...
  const openPost=(id)=>setScreen({type:'post',id})
  ...
  const PostCard=({p,detail=false})=>{const person=personFor(p);const isLike=liked.includes(p.id);return <div style={{background:BASE.surface,border:`1px solid ${BASE.border}`,borderRadius:22,overflow:'hidden',marginBottom:18,boxShadow:'0 8px 24px rgba(66,40,62,.055)'}}>
    ...
    <div onClick={()=>openPost(p.id)} style={{height:detail?310:270,background:p.image,position:'relative',cursor:'pointer'}}><div style={{position:'absolute',inset:0,background:'linear-gradient(180deg,rgba(255,255,255,.04),rgba(45,25,42,.08))'}}/><div style={{position:'absolute',left:16,bottom:14,color:'rgba(255,255,255,.86)',fontFamily:"'Cormorant Garamond', serif",fontStyle:'italic',fontSize:13}}>photo placeholder</div></div>
    <div style={{padding:'12px 15px 15px'}}><div style={{display:'flex',alignItems:'center',gap:6}}><IconButton active={isLike} onClick={()=>toggle(setLiked,liked,p.id)}>{isLike?'♥':'♡'}</IconButton><span style={{fontSize:11.5,color:BASE.taupe}}>{p.likes+(isLike?1:0)}</span><IconButton onClick={()=>openPost(p.id)}>💬</IconButton><span style={{fontSize:11.5,color:BASE.taupe}}>{p.comments.length}</span><div style={{flex:1}}/><IconButton active={saved.includes(p.id)} onClick={()=>toggle(setSaved,saved,p.id)}>{saved.includes(p.id)?'♥':'♡'}</IconButton></div>
      <Source source={p.source}/><div style={{fontSize:13,color:BASE.creamDim,lineHeight:1.55,marginTop:10}}>{p.caption}</div>{!detail&&p.comments.length>0&&<div onClick={()=>openPost(p.id)} style={{fontSize:11.5,color:BASE.taupe,marginTop:10,cursor:'pointer'}}>View {p.comments.length===1?'comment':`all ${p.comments.length} comments`}</div>}</div>
  </div>}
  ...
  if(screen.type==='post'){const p=posts.find(x=>x.id===screen.id);if(!p)return null;return <div className="fade-in" style={{padding:'10px 18px 0'}}><Back/><PostCard p={p} detail/><div style={{fontFamily:"'Cormorant Garamond', serif",fontSize:20,fontWeight:700,color:BASE.cream,margin:'6px 2px 12px'}}>Comments</div>{p.comments.length===0?<div style={{fontSize:12.5,color:BASE.taupe,fontStyle:'italic',padding:'10px 2px 18px'}}>Be the first to say something.</div>:p.comments.map((c,i)=><div key={i} style={{display:'flex',gap:9,padding:'10px 2px',borderBottom:`1px solid ${BASE.border}`}}><div style={{width:28,height:28,borderRadius:'50%',background:'rgba(201,123,168,.18)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:800,color:'#A84E7D'}}>{c.name[0]}</div><div><div style={{fontSize:11.5,fontWeight:800,color:BASE.cream}}>{c.name}</div><div style={{fontSize:12.5,color:BASE.creamDim,lineHeight:1.45,marginTop:2}}>{c.text}</div></div></div>)}<div style={{display:'flex',gap:8,marginTop:15}}><input value={comment} onChange={e=>setComment(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')addComment(p.id)}} placeholder="Add a comment…" style={{flex:1,minWidth:0,padding:'11px 13px',borderRadius:999,border:`1px solid ${BASE.border}`,background:BASE.surface,color:BASE.creamDim,outline:'none'}}/><button onClick={()=>addComment(p.id)} style={{border:'none',background:'transparent',color:'#C9558E',fontWeight:800}}>Post</button></div><div style={{height:70}}/></div>}

  if(screen.type==='profile'){const key=screen.user;const person=PEOPLE[key];const mine=posts.filter(p=>p.user===key);const follows=following.includes(key);return <div className="fade-in" style={{padding:'10px 18px 0'}}><Back/><div style={{textAlign:'center',padding:'8px 15px 22px'}}><div style={{display:'flex',justifyContent:'center'}}><Avatar person={person} size={72}/></div><div style={{fontFamily:"'Cormorant Garamond', serif",fontSize:25,fontWeight:700,color:BASE.cream,marginTop:11}}>{person.name}</div><div style={{fontSize:11,color:BASE.taupe,marginTop:2}}>{person.handle}</div><div style={{fontSize:12.5,color:BASE.creamDim,lineHeight:1.5,margin:'10px auto 14px',maxWidth:290}}>{person.bio}</div><button onClick={()=>toggle(setFollowing,following,key)} style={{padding:'9px 24px',borderRadius:999,border:follows?`1px solid ${BASE.border}`:'none',background:follows?BASE.surface:'linear-gradient(135deg,#D86FA6,#A87BD1)',color:follows?BASE.creamDim:'#fff',fontWeight:800,fontSize:11}}>{follows?'Following':'Follow'}</button></div><div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:3}}>{mine.map(p=><div key={p.id} onClick={()=>openPost(p.id)} style={{aspectRatio:'1/1',background:p.image,cursor:'pointer'}}/>)}</div><div style={{height:70}}/></div>}
