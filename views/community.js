...
const Avatar = ({person,size=38}) => <div style={{width:size,height:size,borderRadius:'50%',background:person.photo?`url(${person.photo}) center/cover no-repeat`:person.gradient,display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',fontFamily:"'Cormorant Garamond', serif",fontWeight:700,fontSize:size*.43,flexShrink:0,overflow:'hidden'}}>{!person.photo&&person.initial}</div>
...
function CommunityApp({ctx}) {
  ...
  const [myProfile,setMyProfile] = useState({name:firstName||'Vanessa',handle:'@yourreverie',bio:'Building a life that feels like mine.',photo:null,reverie:['little joys','feeling like me','home','getting outside']})
  ...
  const chooseProfilePhoto=(e)=>{const file=e.target.files?.[0];if(!file)return;const reader=new FileReader();reader.onload=()=>setProfileDraft(d=>({...d,photo:reader.result}));reader.readAsDataURL(file);e.target.value=''}
