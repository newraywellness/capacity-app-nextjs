...
// automatically above discovery; Feel Better and Rituals live beside Rebuild.
...
export function renderRebuild(ctx) {
  ...
  const RitualCard = ({ r }) => <div style={{borderRadius:22,overflow:"hidden",background:BASE.surface,border:`1px solid ${BASE.border}`,marginBottom:16}}>
    ...
    <div style={{padding:"15px 17px 17px"}}><div style={{fontFamily:"'Cormorant Garamond', serif",fontSize:21,fontWeight:700,color:BASE.cream}}>{r.title}</div><div style={{fontFamily:"'Cormorant Garamond', serif",fontStyle:"italic",fontSize:13.5,color:BASE.taupe,lineHeight:1.5,marginTop:6}}>{r.desc}</div><button onClick={()=>{ if (typeof window!=="undefined") window.scrollTo({top:0,behavior:"instant"}); setRebuildStartWarning({type:r.premium&&!rebuildPlus?"plus-ritual":"ritual-preview",ritual:r}) }} style={{marginTop:13,padding:"9px 14px",borderRadius:999,border:`1px solid ${BASE.border}`,background:BASE.surface,color:"#6B435F",fontSize:11,fontWeight:800}}>Do Ritual</button></div>
  </div>
  ...
  if (!rebuildActiveProgram) {
    ...
    return <div className="fade-in" style={{padding:"10px 18px 0"}}>
      ...
      <div style={{display:"flex",gap:7,marginTop:18,marginBottom:22}}>{[["rebuild","Rebuild"],["feel-better","Feel Better"],["rituals","Rituals"]].map(([k,l])=><button key={k} onClick={()=>setRebuildSection(k)} style={{flex:1,padding:"10px 8px",borderRadius:14,border:`1px solid ${rebuildSection===k?"#C97BA8":BASE.border}`,background:rebuildSection===k?"rgba(201,123,168,.12)":BASE.surface,color:rebuildSection===k?"#A84E7D":BASE.taupe,fontWeight:800,fontSize:11.5}}>{l}</button>)}</div>
      ...
      {rebuildSection==="feel-better"&&<>
        <div style={{fontFamily:"'Cormorant Garamond', serif",fontSize:22,fontWeight:700,color:BASE.cream}}>What would make today feel better?</div>
        <div style={{fontSize:11.5,color:BASE.taupe,lineHeight:1.55,margin:"6px 0 17px"}}>Not a program. Just choose what you need right now and take one small step toward it.</div>
        {[
          {id:"calmer",ic:"☁️",title:"I need to feel calmer",desc:"Turn the volume down for a minute.",action:"Put your phone down, unclench your jaw, and take five slow breaths. Then make the next ten minutes quieter than the last ten."},
          {id:"energy",ic:"☀️",title:"I need a little energy",desc:"Wake your body up without making it a workout.",action:"Drink something cold, open a window or step outside, and move for one song. That's enough to change the temperature of the moment."},
          {id:"human",ic:"🫧",title:"I want to feel human again",desc:"A tiny care reset for when you've disappeared from your own day.",action:"Wash your face, brush or fix your hair, change into something clean, and use one thing that smells good. Stop there if that's all you have."},
          {id:"home",ic:"🕯️",title:"My space is making me feel worse",desc:"Make one small area easier to exist in.",action:"Pick the surface you can see most. Clear only that, throw away obvious trash, and add one comforting thing — a lamp, candle, blanket, or open window."},
          {id:"stuck",ic:"🌿",title:"I feel stuck",desc:"Give the day one new direction.",action:"Choose one thing you've been circling and do the first five-minute piece. You do not have to finish it; you only have to interrupt the stuck feeling."},
          {id:"something",ic:"✨",title:"I just need something nice",desc:"No fixing. No productivity. Just a little pleasure.",action:"Pick the easiest thing that sounds genuinely good: a favorite drink, shower, music, outside air, a snack, a show, or texting someone you actually want to talk to."},
        ].map((x)=><button key={x.id} onClick={()=>{ if(typeof window!=="undefined") window.scrollTo({top:0,behavior:"instant"}); setRebuildStartWarning({type:"feel-better",feel:x}) }} style={{width:"100%",textAlign:"left",display:"flex",gap:13,alignItems:"center",padding:"16px 15px",marginBottom:10,borderRadius:18,border:`1px solid ${BASE.border}`,background:BASE.surface,cursor:"pointer"}}><div style={{width:38,height:38,borderRadius:14,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(201,123,168,.10)",fontSize:19,flex:"0 0 auto"}}>{x.ic}</div><div><div style={{fontFamily:"'Cormorant Garamond', serif",fontSize:17,fontWeight:700,color:BASE.cream}}>{x.title}</div><div style={{fontSize:11.5,color:BASE.taupe,lineHeight:1.45,marginTop:3}}>{x.desc}</div></div></button>)}
      </>}
