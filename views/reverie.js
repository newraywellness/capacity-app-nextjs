...
export function renderReverie(ctx) {
  const { tab, reverieSection, setReverieSection, reverieEntries, setReverieEntries, reverieDraft, setReverieDraft, reverieComposerOpen, setReverieComposerOpen, reverieSearch, setReverieSearch, savedBloom, rebuildSaved, woLog, foodDays, progress, rebuildFLYA, likedFeed } = ctx
  if (tab !== 'reverie') return null

  const today = new Date().toISOString().slice(0,10)
  const entries = [...(reverieEntries || [])].sort((a,b)=>(a.date < b.date ? 1 : -1))
  const withPhotos = entries.filter(x=>x.photo)
  ...
  const saved = [
    ...(savedBloom || []).map((id)=>({ id:'b-'+id, title:String(id).replace(/[-_]/g,' '), source:'Bloom' })),
    ...(rebuildSaved || []).map((id)=>({ id:'r-'+id, title:String(id).replace(/[-_]/g,' '), source:'Rebuild' }))
  ]
  ...
  const openComposer = () => { setReverieDraft({ title:'', note:'', date:today, photo:null, share:false }); setReverieComposerOpen(true) }
  const saveEntry = () => {
    if (!reverieDraft.title.trim() && !reverieDraft.photo) return
    const next = [...(reverieEntries||[]), { id:'life-'+Date.now(), title:reverieDraft.title.trim() || 'A moment from my life', note:reverieDraft.note.trim(), date:reverieDraft.date || today, photo:reverieDraft.photo || null, source:'personal', share:!!reverieDraft.share }]
  ...
  const Search = ({placeholder}) => <div style={{display:'flex',alignItems:'center',gap:9,border:`1px solid ${BASE.border}`,background:BASE.surface,borderRadius:16,padding:'12px 14px',marginBottom:18}}><span style={{fontSize:14}}>⌕</span><input value={reverieSearch} onChange={e=>setReverieSearch(e.target.value)} placeholder={placeholder} style={{border:'none',outline:'none',background:'transparent',width:'100%',fontSize:13,color:BASE.cream}}/></div>

  const PhotoStrip = () => (
    <div style={{display:'flex',gap:12,overflowX:'auto',paddingBottom:8,scrollSnapType:'x mandatory',WebkitOverflowScrolling:'touch'}}>
      {withPhotos.slice(0,10).map(e=><div key={e.id} style={{minWidth:190,scrollSnapAlign:'start'}}><div style={{height:230,borderRadius:20,overflow:'hidden',background:'#F1E5EF'}}><img src={e.photo} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/></div><div style={{fontFamily:FONT,fontSize:17,fontWeight:600,marginTop:9,color:BASE.cream}}>{e.title}</div><div style={{fontSize:11,color:BASE.taupe,marginTop:2}}>{fmtDate(e.date)} · {sourceLabels[e.source]||e.source}</div></div>)}
  ...
  return <div className="fade-in" style={{padding:'24px 18px 30px',color:BASE.cream}}>
    ...
    {reverieSection==='home' && <>
      <div style={{fontSize:10,fontWeight:800,letterSpacing:2.2,color:BASE.taupe,textTransform:'uppercase'}}>Your Taste</div>
      ...
      {withPhotos.length ? <PhotoStrip/> : <EmptyScrapbook/>}
    ...
    {reverieSection==='saved' && <><div style={{fontFamily:FONT,fontSize:28,fontWeight:600}}>Saved</div><div style={{fontSize:12.5,color:BASE.taupe,lineHeight:1.5,margin:'5px 0 17px'}}>Everything you wanted to come back to, in one place.</div><Search placeholder="Search your saves"/>{filteredSaved.length?<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>{filteredSaved.map(x=><div key={x.id} style={{minHeight:150,borderRadius:18,background:'linear-gradient(145deg,#F4E6F2,#E9E4F4)',padding:14,display:'flex',flexDirection:'column',justifyContent:'flex-end'}}><div style={{fontFamily:FONT,fontSize:18,fontWeight:600,textTransform:'capitalize'}}>{x.title}</div><div style={{fontSize:10,color:BASE.taupe,marginTop:4,textTransform:'uppercase',letterSpacing:1}}>{x.source}</div></div>)}</div>:<div style={{padding:'35px 20px',borderRadius:20,background:'#fff',border:`1px solid ${BASE.border}`,textAlign:'center'}}><div style={{fontFamily:FONT,fontSize:22,fontWeight:600}}>Nothing saved here yet.</div><div style={{fontSize:12.5,color:BASE.taupe,lineHeight:1.55,marginTop:7}}>Save something anywhere in True Reverie and this becomes the place to find it again.</div></div>}</>}

    {reverieSection==='history' && <><div style={{display:'flex',justifyContent:'space-between',alignItems:'end'}}><div><div style={{fontFamily:FONT,fontSize:28,fontWeight:600}}>History</div><div style={{fontSize:12.5,color:BASE.taupe,lineHeight:1.5,marginTop:5}}>Things you’ve actually brought into your life.</div></div><button onClick={openComposer} style={{border:'none',background:'#FBEAF2',color:pink,borderRadius:999,padding:'9px 12px',fontSize:11,fontWeight:700}}>＋ Add</button></div><div style={{marginTop:17}}><Search placeholder="Search what you’ve done"/></div>{filteredHistory.length?<div>{filteredHistory.map(e=><div key={e.id} style={{display:'flex',gap:13,padding:'12px 0',borderBottom:`1px solid ${BASE.border}`}}>{e.photo?<img src={e.photo} alt="" style={{width:72,height:72,borderRadius:14,objectFit:'cover'}}/>:<div style={{width:72,height:72,borderRadius:14,background:'linear-gradient(145deg,#F4E6F2,#E9E4F4)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:22}}>✧</div>}<div style={{flex:1,paddingTop:3}}><div style={{fontFamily:FONT,fontSize:18,fontWeight:600}}>{e.title}</div><div style={{fontSize:10.5,color:BASE.taupe,marginTop:3}}>{fmtDate(e.date)} · {sourceLabels[e.source]||e.source}</div>{e.note&&<div style={{fontSize:12,color:BASE.creamDim,lineHeight:1.45,marginTop:6}}>{e.note}</div>}{e.share&&<div style={{fontSize:10.5,color:pink,marginTop:6}}>Ready to share to Community</div>}</div></div>)}</div>:<div style={{padding:'34px 20px',borderRadius:20,background:'#fff',border:`1px solid ${BASE.border}`,textAlign:'center'}}><div style={{fontFamily:FONT,fontSize:22,fontWeight:600}}>Your history starts with living.</div><div style={{fontSize:12.5,color:BASE.taupe,lineHeight:1.55,margin:'7px 0 17px'}}>Add something you did—even if True Reverie never suggested it.</div><button onClick={openComposer} style={{border:'none',borderRadius:999,padding:'11px 15px',background:grad,color:'#fff',fontWeight:700,fontSize:11.5}}>＋ Add something I did</button></div>}</>}
    <Composer/>
  </div>
}
