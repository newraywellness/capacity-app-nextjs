...
export function renderReverie(ctx) {
  ...
  const readPhoto = (file) => {
    if (!file) return
    const r = new FileReader(); r.onload = () => setReverieDraft(d=>({...d,photo:r.result,photoPosition:{x:50,y:50}})); r.readAsDataURL(file)
  }
