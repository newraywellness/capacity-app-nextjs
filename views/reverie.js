...
export function renderReverie(ctx) {
  ...
  return <div className="fade-in" style={{padding:'24px 18px 30px',color:BASE.cream}}>
    ...
    {reverieSection==='saved' && <><div style={{fontFamily:FONT,fontSize:28,fontWeight:600}}>Saved</div><div style={{fontSize:12.5,color:BASE.taupe,lineHeight:1.5,margin:'5px 0 17px'}}>Everything you wanted to come back to, in one place.</div><Search placeholder="Search your saves"/>{filteredSaved.length?<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>{filteredSaved.map(x=><div key={x.id} onClick={x.onOpen} role="button" tabIndex={0} style={{minHeight:150,borderRadius:18,background:'linear-gradient(145deg,#F4E6F2,#E9E4F4)',padding:14,display:'flex',flexDirection:'column',justifyContent:'flex-end',cursor:'pointer'}}><div style={{fontFamily:FONT,fontSize:18,fontWeight:600,textTransform:'capitalize'}}>{x.title}</div><div style={{fontSize:10,color:BASE.taupe,marginTop:4,textTransform:'uppercase',letterSpacing:1}}>{x.source}</div></div>)}</div>:<div style={{padding:'35px 20px',borderRadius:20,background:'#fff',border:`1px solid ${BASE.border}`,textAlign:'center'}}><div style={{fontFamily:FONT,fontSize:22,fontWeight:600}}>Nothing saved here yet.</div><div style={{fontSize:12.5,color:BASE.taupe,lineHeight:1.55,marginTop:7}}>Save something anywhere in True Reverie and this becomes the place to find it again.</div></div>}</>}
